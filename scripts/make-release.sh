...
function version_bump() {
  dev_version_bump $1
  # NOTE: fpm uses semver v1; https://github.com/fortran-lang/fpm/issues/518
  sed -i "s|version = \".*\"|version = \"${1%-*}\"|" fpm.toml
...
current_version=$(grep Version $main_file | head -1 | sed 's/! *Version//' || :)
