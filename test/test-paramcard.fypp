module test_get_param
    ...
    public :: collect_test_get_param

contains

    subroutine collect_test_get_param(testsuite)
        type(unittest_type), allocatable, intent(out) :: testsuite(:)

        testsuite = [ &
                    new_unittest("get_param (basic)", test_get_param_basic) &
                    ]
    end subroutine collect_test_get_param

    subroutine test_get_param_basic(error)
        ...
        call check(error, param_changed())
        if (allocated(error)) return
    end subroutine test_get_param_basic
...
end module test_get_param

program tester
    use, intrinsic :: iso_fortran_env, only: error_unit
    use testdrive, only: run_testsuite, new_testsuite, testsuite_type
    use test_get_param, only: collect_test_get_param
    implicit none
    integer :: stat, is
    ...
    testsuites = [ &
                 new_testsuite("util", collect_test_get_param) &
                 ]
